...
class HDFutils(object):
    ...
    def create (self):
        ...
        class OBook(IsDescription):
            ...
            bids      = Int32Col(shape=(self.depth,))   # integer
            bidsize   = Int32Col(shape=(self.depth,))   # integer
            asks      = Int32Col(shape=(self.depth,))   # integer
...
class ParserCSV(object):
    def __init__(self,file,**kwargs):
        ...
        self.rowheader = kwargs.get('rowheader',False)
        self.headers = (Message.APPLVERID, Message.BODYLENGTH, Message.LASTMSGSEQNUMPROCESSED, Message.MDSECURITYTRADINGSTATUS, Message.MSGSEQNUM, Message.MSGTYPE, Message.POSSDUPFLAG, Message.SENDERCOMPID, Message.SENDINGTIME)
        self.fields = ( Message.AGGRESSORSIDE, Message.FIXINGBRACKET, Message.MATCHEVENTINDICATOR, Message.MDENTRYPX, Message.MDENTRYSIZE, Message.MDENTRYTIME, Message.MDENTRYTYPE, Message.MDPRICELEVEL, Message.MDQUOTETYPE, Message.MDUPDATEACTION, Message.NETCHGPREVDAY, Message.NOMDENTRIES, Message.NUMBEROFORDERS, Message.OPENCLOSESETTLEFLAG, Message.QUOTECONDITION, Message.RPTSEQ, Message.SECURITYDESC, Message.SECURITYID, Message.SECURITYIDSOURCE, Message.SETTLDATE, Message.TICKDIRECTION, Message.TRADECONDITION, Message.TRADEDATE, Message.TRADEVOLUME, Message.TRADINGSESSIONID)
        self.writemode = kwargs.get('writemode','a')

    def toCSV(self):
        with open(self.file, "r") as fh, open(self.output,self.writemode) as oh:
            if self.rowheader:
                oh.write("APPLVERID,BODYLENGTH,LASTMSGSEQNUMPROCESSED,MDSECURITYTRADINGSTATUS,MSGSEQNUM,MSGTYPE,POSSDUPFLAG,SENDERCOMPID,SENDINGTIME,AGGRESSORSIDE,FIXINGBRACKET,MATCHEVENTINDICATOR,MDENTRYPX,MDENTRYSIZE,MDENTRYTIME,MDENTRYTYPE,MDPRICELEVEL,MDQUOTETYPE,MDUPDATEACTION,NETCHGPREVDAY,NOMDENTRIES,NUMBEROFORDERS,OPENCLOSESETTLEFLAG,QUOTECONDITION,RPTSEQ,SECURITYDESC,SECURITYID,SECURITYIDSOURCE,SETTLDATE,TICKDIRECTION,TRADECONDITION,TRADEDATE,TRADEVOLUME,TRADINGSESSIONID\n")
            for line in fh:
                msg = Message(line)
                if not msg.isIncremental(): continue
                msg.parse()
                msgheader =  ",".join([ msg.header.get(k,"") for k in self.headers ])
                mdentries= msg.getMDENTRIES()
                for entry in mdentries:
                    if self.symbol is 'ALL':
                        z = [entry.get(j,"") for j in self.fields if int(entry.get(Message.MDPRICELEVEL,100)) >= self.depth]
                        if len(z):
                            rec=  msgheader +"," + ",".join(z) +"\n"
                            oh.write(rec)
                    elif entry.get(Message.SECURITYDESC) == self.symbol:
                        z = [entry.get(j,"") for j in self.fields if int(entry.get(Message.MDPRICELEVEL,0)) <= self.depth]
                        if len(z):
                            rec= msgheader +","+ ",".join(z) +"\n"
                            oh.write(rec)
...
class Parser(object):
    def __init__ (self,file,**kwargs):
        ...
        self.pp = pprint.PrettyPrinter(depth=4)
        self.hdfutils = HDFutils(kwargs.get('hdf5file'),self.depth)
...
if __name__ =='__main__':
    ...
    parser = argparse.ArgumentParser(description='CME FIX Message Parser'
                                ,epilog="Example: python Parser.py -s ESH1 -f ../data/XCME_ES_ES_FUT_20110110/XCME_ES_ES_FUT_20110110.txt.002 \n" )
    parser.add_argument("-f","--file",type=str,required=True, help="File to parse")
    parser.add_argument("-o", "--output", type=str ,help="output file",default='/tmp/cmefix.csv')
    parser.add_argument("-s", "--symbol", type=str,default='ALL' ,help="symbol ie. ESH1 defaults to all symbols")
    parser.add_argument("-m", "--marketdepth", type=int ,help="market depth levels defaults to maximum 10", default=10)
    parser.add_argument('-d','--dryrun',help="display object args", action='store_true')
    parser.add_argument('-r','--rowheader',help="with header row", action='store_true')
    parser.add_argument('-w','--writemode',choices=['a','w'],help="write output mode. default: append", default='a')
    ...
    p = ParserCSV(args.file
                ,symbol=args.symbol
                ,depth= min(args.marketdepth,10)
                ,output=args.output
